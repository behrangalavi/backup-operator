...
- **mysqldump flag hardening: `--events --default-character-set=utf8mb4` always, `--column-statistics=0` only when the binary supports it.** `--events` was missing — restoring a dump made without it loses any MySQL Event Scheduler entries the application depends on, with no warning. `--default-character-set=utf8mb4` forces a 4-byte session charset; the legacy "utf8" default in MySQL <8 is 3-byte and silently truncates emoji and some CJK characters at restore time. `--column-statistics=0` disables Oracle MySQL 8's client probe of `information_schema.column_statistics` that fails outright against MariaDB / MySQL <8 — but `mariadb-dump` does not know the flag and aborts with `unknown variable 'column-statistics=0'`. We probe `<binary> --help` once per binary path per worker process and only pass the flag when it's actually accepted.

- **Per-engine dump binary: `mysqldump` for MySQL sources, `mariadb-dump` for MariaDB.** Both speak the MySQL wire protocol but the canonical tools differ. The worker image ships both (debian-slim base + Oracle's MySQL Community APT for `mysql-community-client` + the standard `mariadb-client` package). The mysql dumper picks the binary by source `dbType`. Trade-offs considered: (a) shipping only mariadb-client (the previous alpine-based design) was wire-protocol-correct for MySQL targets but produced MariaDB-flavoured dumps with MariaDB banners, missing MySQL-8-specific bits like column histograms and `INVISIBLE INDEX` correctness — and broke when a MySQL-8 flag like `--column-statistics=0` was hard-coded; (b) a glibc-shim hack to run Oracle's mysqldump on alpine would work for the 95% case but fragile across Oracle patch releases. Switching the runtime base to debian-slim (~280MB amd64, ~530MB total with all clients vs. ~100MB on alpine) is the price of honest engine separation for a tool whose job is correctness, not throughput. **arm64 caveat:** Oracle's MySQL Community APT does not ship arm64 binaries, so on arm64 builds (typically local dev on Apple Silicon) the image installs `mariadb-client` only and `mysqldump` resolves to `mariadb-dump` — same situation as the pre-debian image. Production amd64 builds get the real Oracle client. The dumper's binary-probe handles both transparently.
