...
|---|---|---|
| `backup.mogenius.io/name` | Secret name | Logical target name. Used in metrics labels, object paths, CronJob naming. Because it flows into storage object paths via `path.Join`, it is validated at parse time against `^[A-Za-z0-9][A-Za-z0-9._-]*$` (no `/`, no leading `.`, ≤253 chars) — a value like `../other-tenant` is rejected and the Secret is skipped, unlike feature-flag annotations which fall back to a default. |
...
| `backup.mogenius.io/verification-image` | per-DB-type default | Container image for the verifier pod. Only consulted in Phase-2 modes. Pin this to match the source DB's exact major version when restore semantics depend on the engine version (charset defaults, function signatures, dump format compatibility). Per-DB defaults from `verifier/restore/engine.go.DefaultImage`: `postgres:16-alpine`, `mysql:8.0`, `mariadb:11`, `mongo:7`, `redis:7-alpine`. **The image's non-root DB user UID must match what the spawner runs the pod as** (`runAsUIDForImage`): the pod runs under `runAsNonRoot`, so a UID that doesn't own the image's DB user makes the entrypoint fail "could not look up effective user ID" and the pod never becomes ready. The spawner auto-selects `70` for `postgres:*-alpine` and `999` for a Debian postgres and for mysql/mariadb/mongo/redis — so a postgres override is safe across alpine↔Debian, but an override to an image with a different DB-user UID (a custom build) will not restore. |
...
| `database` | (situational) | Postgres/MySQL/MariaDB: required for `pg_dump`/`mysqldump` to scope. Mongo: optional, omitted = all non-system DBs. Redis: optional DB index or **comma-separated list** of indexes (e.g. `0` or `0,3,7`) — narrows *stats* only; the RDB dump is always full-instance. |
...
| `backup.mogenius.io/path-prefix` | Prefix prepended to every uploaded object path. May contain `/` (it is the documented tenant/cluster isolation root), but a `..` path segment is rejected at parse time so one destination's prefix cannot climb into another's namespace. The logical destination `name` is validated with the same rule as the source target name. |
...
| `backup_operator_last_dump_duration_seconds` | Gauge | `target`, `db_type` | Duration of the **dump phase alone** of the most recent successful run, reconstructed from `meta.json`'s `dumpDurationSeconds`. Same operator-side reconstruction as `last_run_duration_seconds`; lets you separate dump time from upload/retention time. The worker-side `dump_duration_seconds` histogram is never scraped. |
...
| `backup_operator_destination_failed` | Gauge | `target`, `destination` | 1 if the destination's storage cannot be initialised **or cannot be reached/read at refresh time** (offline host, rotated credentials, unreadable meta), 0 once the destination is reachable (a meta.json was read, or the target prefix listed cleanly with nothing uploaded yet) |
...
| `backup_operator_source_parse_errors_total` | Counter | `secret` | Source Secrets that failed annotation/data parsing during reconciliation. Incremented by both the CronJob reconciler and the MetricsRefresher when a still-present Secret can't be parsed (e.g. an invalid name annotation, a required key cleared mid-edit). Operator-side, scraped normally. Note: the refresher keeps a target's other series sticky across a transient parse error, so this counter is the signal that a Secret is misconfigured even while its gauges look healthy. |
| `backup_operator_http_request_duration_seconds` | Histogram | `method`, `path`, `status` | UI HTTP request latency. `path` is normalised to a closed label set (`normalizeMetricPath`) — parametric routes collapse to their prefix, unknown paths to `"other"` — so an unauthenticated path-scanner can't blow up series cardinality. |
...
| Dump uploads succeed but `meta.json` upload fails on **all** destinations (transient outage outlasting 3 retries during the small meta upload) | Worker now treats this as a run failure: `SetLastRunStatus(false)`, `BackupFailed` Event, worker exits 1 → K8s retries with a fresh timestamp. Previously reported success silently (phantom backup: dump exists, no sidecar) | Job failed; the dump objects from the failed attempt are orphaned (no meta) until retention prunes them. A partial meta failure (sidecar reached ≥1 destination) keeps the run successful and emits `BackupMetaPartial` (warning) instead |
| `CollectStats` fails (no perms) | Analyzer skips, dump still succeeds | meta.json still written without stats; `schema_changed` / `table_count` stay at their last values. With `empty-dump-check` on, the run also emits `BackupEmptyCheckDegraded` (warning) because the empty-dump heuristic needs a pre-dump stats baseline and silently can't fire without one |
...
| Role label removed OR changed `source`→`destination` (Secret kept) | Reconciler observes the transition and deletes the CronJob — but only after confirming the CronJob carries an OwnerReference back to this Secret's UID (a same-named CronJob the operator didn't create is left untouched) | Same as above for scheduling |
...
|---|---|---|
| Worker → Database | DB-native (pg, mysql, mongo, redis) | TLS if configured via `extra-sslmode` (postgres) / `extra-tls` (mysql/mariadb; also redis — `extra-tls=true` adds `redis-cli --tls`). For pg/mysql/mariadb it applies to BOTH the stats connection AND the dump tool (`pg_dump` via `PGSSLMODE`, `mysqldump`/`mariadb-dump` via `--ssl-mode`/`--ssl`); the mysql stats connection maps `verify-*` modes to a valid go-sql-driver value (identity modes → system-root verification, other require-TLS values → encrypted-without-identity) while the dump path still verifies against `extra-ssl-ca`. Certificate verification (`verify-ca`/`verify-full`) additionally needs `extra-ssl(root)cert`/`extra-ssl-ca` pointing at a CA file mounted into the worker pod; without one, an enabled TLS value still enforces an encrypted channel. |
...
| `BackupMetaPartial` | Worker pod | Same; Warning. The `meta.json` sidecar reached some but not all destinations whose dump succeeded — run is still discoverable, but per-destination history diverges. |
| `BackupEmptyCheckDegraded` | Worker pod | Same; Warning. `empty-dump-check` is on but no pre-dump stats baseline was available, so the empty-dump heuristic could not run for this run. |
...
- **Label-transition predicate.** The reconciler does need to handle `role` label *removal* (Secret kept, label dropped) — that's an explicit user signal "stop backing this up." Without watching label transitions, an orphan CronJob would persist. The same cleanup runs on a `source`→`destination` relabel: the `RoleDestination` reconcile branch is not a no-op — it routes through the ownership-checked delete, because a Secret that was a source still owns the CronJob the source branch created, and leaving it would keep spawning worker Jobs against a Secret that is now a destination.

- **CronJob deletion is ownership-checked, not name-only.** `deleteCronJobIfOwned` used to `Delete` `backup-<secretName>` by name outright. The name is not proof of ownership: an operator running their own unrelated CronJob that happens to collide with that name (or a hash-suffix collision between two long Secret names mapping to the same managed name) would have their CronJob destroyed the moment a same-named Secret lost its role label. The fix Gets the object first, verifies a controller OwnerReference back to *this* Secret's UID (`ownedBySecret`, the same reference `buildCronJob` sets and K8s GC keys on), and deletes under a `client.Preconditions{UID}` so a same-named object recreated between Get and Delete is not clobbered. UID — not name, not the managed-by label — is the authoritative signal, since a foreign object cannot forge an OwnerReference to a UID it doesn't know. Regression-tested: owned→deleted, unowned→left, wrong-owner-UID→left.
...
- **Three binaries, one image.** Two-binary distribution per service is awkward. A single image with two `cmd/` entrypoints means one CI build, one registry tag, one version to track. Revisited the split question: the image is ~530 MB, ~430 MB of which is DB client tooling (`pg_dump` from PGDG, MySQL/MariaDB client, mongo-tools) that *only the worker* uses — the operator never shells out to a dump tool. A slim operator image (~100 MB) was considered and rejected. The headline benefit (faster operator pulls) largely evaporates because the worker mandates the fat image on every node it runs on (cached via `IfNotPresent`), so a separate operator image *adds* an image to the node rather than replacing one — node disk goes up, not down, unless operator and worker land on disjoint nodes. The security argument is also weak: the operator already holds Secret CRUD (the DB credentials themselves), so a `pg_dump` binary sitting next to them is a marginal attack-surface increase against what a compromised operator can already do. Against those near-zero gains sit permanent distribution costs: two tags, two Helm image refs, doubled digest pinning, version-sync in the release workflow. If image size becomes a real problem, the right lever is trimming the *single* image (drop unused client packages, `--no-install-recommends`, layer cleanup) — that shrinks it for both pods without the split's complexity. (The original "30 MB binary-size difference is irrelevant" rationale was on the wrong axis — the real cost was never the binaries, it was the client tooling, and the conclusion still holds once you reason about node-level image caching.)
...
- **Per-destination and per-table series go absent on removal, not sticky.** The `MetricsRefresher` already dropped a target's whole series set when its Source Secret vanished (`trackedTargets` → `DeleteTargetMetrics`), but it had no equivalent for the *sub-target* label dimensions. When a destination was dropped from a source's `backup.mogenius.io/destinations` allow-list, its `retention_last_status{target,destination}` / `destination_failed{...}` / scrub series kept reporting their last value forever — a removed backup target looked permanently healthy (or permanently failing). Same for a table dropped from the schema: `table_row_count{target,table}` stuck at the last count, so a deleted table's rows never went to zero, they just froze. The refresher now tracks, per target, the destination and table label-sets it exposed last tick (`trackedSeries`), and on the next tick deletes the series for any destination no longer in the allow-list (`DeleteDestinationMetrics`) or any table no longer in the schema (`DeleteTableMetric`). Destination reconciliation runs up front from the source's allow-list (a pure config function, independent of whether run data exists yet), so a removed destination is cleaned even when the remaining destinations are currently empty. Table reconciliation runs only on a tick that has *authoritative* stats (a successful run with a `Stats` block) — a failed run leaves the prior table set sticky rather than spuriously deleting every table because stats collection was skipped. Trade-off accepted: this is "absent" semantics (the series disappears), which means PromQL `... unless` / `absent()` queries see the change but a naive `== 0` alert won't — consistent with how `DeleteTargetMetrics` already behaves for whole-target removal.

- **Run duration as a Gauge from meta.json, not as a histogram via Pushgateway.** Run timing was a known observability gap: `run_duration_seconds` (histogram) is observed in the worker but never reaches Prometheus. Reviewers naturally suggested filling it with Pushgateway or OTel. We picked the same pattern the operator-side aggregator already uses for `last_run_status` / `dump_size_bytes`: read `DurationSeconds` out of the most recent successful meta.json and expose it as `last_run_duration_seconds{target,db_type}`. Cost is one Gauge declaration plus three lines in `MetricsRefresher.refreshSource`; the meta.json field already existed (it powers the UI's progress-bar median estimate). Trade-off explicitly accepted: no distribution metric. P95/P99 across runs is not directly available — `quantile_over_time(0.95, last_run_duration_seconds[7d])` is a quantile *over time samples* of the last-run gauge, not over runs. For "is this run slower than usual" the gauge is sufficient (`last_run_duration_seconds / avg_over_time(last_run_duration_seconds[7d]) > 3`); for genuine SLO-style distribution analysis a future OTel export remains the right path. Failed runs are excluded — same logic as the UI's `MedianDuration` and `dump_size_bytes`: failures fail fast (connection refused in <1 s) and would systematically underestimate. **Update:** the dump phase is now broken out too — `meta.json` carries `dumpDurationSeconds` and the refresher reconstructs `last_dump_duration_seconds{target,db_type}` the same way, so dump time is separable from upload/retention time. Only the per-destination `upload_duration_seconds` histogram remains unexposed (meta.json records per-destination *results* but not per-destination upload *durations*); giving it the same treatment would need a meta.json schema extension and is deferred until a concrete need surfaces.
...
- **Two-tier error sanitization: fully-generic vs credential-scrubbed.** A blanket "collapse every error to `operation failed`" rule collided with the test-connection ADR below, which deliberately surfaces protocol diagnostics like `550 Permission denied`. The resolution is two helpers: `sanitizeError` (fully generic — for data-fetch paths like `/download/...` where no diagnostic value is intended) and `sanitizeStorageError` (`errors_api.go` — strips credential material but keeps the protocol message — for the diagnostic endpoints `test-connection`, `destination-stats`, `destination-health`). The scrubber masks `scheme://user:pass@host` userinfo and `password=`/`auth=`/`token=`/`secret-access-key=`/`access-key-id=` key-value pairs via two regexes, leaving the operationally useful tail intact. Before this, several diagnostic handlers returned raw `err.Error()`, contradicting the sanitization ADR; the download handler leaked storage internals into a `502` body and now logs server-side + returns a fixed message.
...
- **Pool lazy-init guarded by `sync.Once`; SFTP keepalive goroutine bound to connection lifetime.** Two concurrency hardening fixes on the operator-side storage path. (1) `StorageScrubber` and `MetricsRefresher` both lazily build their `StoragePool` when none is wired (tests, non-production paths). The old `if s.Pool == nil { s.Pool = NewStoragePool(...) }` ran inside `scrub`/`refresh`, which fan out to concurrent per-destination goroutines — the unguarded nil-check + assign was a data race on the lazy path (the `go test -race` flagged it only intermittently because production wires the pool externally). Replaced with an `ensurePool()` that does the build inside `poolOnce.Do`, idempotent and race-free; an externally-wired pool is left untouched. (2) The SFTP `dial` keepalive goroutine watched only `ctx.Done()` and a 30s ticker. For a pooled `sftpStorage` dialed under the operator's long-lived context, the client can be closed (pool eviction, credential rotation, server drop) while ctx is still alive — the goroutine then survived until the next tick happened to fail `SendRequest`, leaking one goroutine per dial for up to a full interval. Fix: a second goroutine blocks on `client.Wait()` and closes a `closed` channel the instant the connection dies; the keepalive loop selects on `closed` (plus ctx for prompt shutdown), tying its lifetime exactly to the connection. Note: the earlier ADR's "future Reconnect-on-Error wrapper at the pool layer" is moot for SFTP/FTPS as currently written — both `storage.Storage` impls dial a fresh connection per call (`dial(ctx)` inside every Upload/List/Get/Delete), so the pool caches only the config object, not a live connection. There is no persistent pooled connection to go stale; liveness is inherent in the per-call dial. The pool's handshake-saving benefit applies only via the `WithSession` batch path (retention), not the per-call metric/scrub reads.

- **MetricsRefresher/scrubber hot path reuses one SFTP/FTPS session for List+Get via `WithSession`.** `loadLatestMeta` runs once per (source × destination) every refresher and scrubber tick, and it does a `List` to find the newest meta followed by a `Get` to read it. Because the SFTP/FTPS `storage.Storage` impls dial a fresh connection per call, that was two SSH/TLS handshakes per pair per tick — the dominant cost on the hot path (handshake ≫ the 5–50 KB meta payload, see the MetricsRefresher ADR). Fix: type-assert the storage to `storage.BatchStorage`, open one `WithSession`, and route both the List and the Get through it so the pair shares a single connection. S3 has no session concept and falls through to per-call dialing unchanged. Mirrors the existing `WithSession` use in retention. Defer ordering is load-bearing: the session `closer()` is registered before the `rc.Close()` for the fetched object, and `io.ReadAll(rc)` completes before either fires (LIFO: `rc.Close()` runs first, then `closer()`), so the body is fully read while the session is still open.

- **No `f.Sync()` on the dump temp file.** `dumpToFileWithEncryptor` used to fsync the temp file before `Stat`. The file lives on the worker pod's `emptyDir` and is deleted at the end of the run (`defer os.Remove`), so it never needs to survive a crash — fsync bought durability we explicitly don't want and added latency proportional to dump size. Reads (the fan-out re-opens the file, the verifier re-streams it) go through the page cache and see the written bytes regardless of whether they've hit disk, so correctness is unaffected. Removed.

- **Bounded UI background-refresh fan-out via a shared semaphore.** The UI's stale-while-revalidate caches (`latestCache`, `runsCache`) dedup background refreshes *per key* via the `loading` map, but had no bound *across* keys. On a fleet with many distinct destinations, one SSE tick could fan out one refresh goroutine per key at once — each a fresh SSH/TLS handshake — which is exactly the connection storm that got the operator IP-blocked by QNAP's bruteforce protection (the test-destination ADR). Fix: a single `sem chan struct{}` (cap `maxBackgroundRefreshes`=6) shared by both caches, acquired inside the background goroutine in `getOrRefreshAsync` and released on completion, so the cap is on *total* in-flight probes, not per-cache or per-key. `nil` sem = unbounded (preserves behaviour for any cache constructed without one). Per-key dedup still runs first, so the semaphore only ever sees distinct-key probes. Trade-off: when more than 6 destinations need refreshing at once, the surplus probes queue rather than run concurrently — a deliberately slower-but-safe fan-out, and the SSE-driven repaint still lands fresh data as each completes.

- **Post-dump `CollectStats` only for engines without a row counter (mongo/redis).** The pipeline collected stats both before AND after the dump. `postStats` feeds exactly one path in `BuildVerification`: the `!hasDump && hasPre && hasPost` branch, which is the *only* verdict path for mongo/redis (their archive/RDB formats can't be row-counted in-stream). For SQL engines the row counter already saw every INSERT/COPY row, so the verdict comes from `dumpCounts`-vs-`preStats` and `postStats` is purely informational meta content (`PostStats` / per-table `PostDumpRows`) that nothing downstream consumes for a decision — not the analyzer (it compares against `preStats`), not any metric, not the UI. Empty-dump detection for SQL also uses only `preStats` + `dumpCounts`. So gating the second collection on `!rowCounter.Active()` saves a full `CollectStats` roundtrip (a `pg_stat_user_tables` / `INFORMATION_SCHEMA` query) on every SQL backup — real load at 10k+ daily runs — with a verdict-identical result. Observable change: SQL `meta.json` no longer carries the `postStats` block (it's `omitempty`), but the stream-observed `dumpRowCounts` it keeps are more authoritative than a post-dump estimate anyway. Mongo/redis behaviour is byte-identical.
...
- **Sensitive field masking in API GET responses.** Passwords, SSH private keys, and S3 secret keys are returned as `***` in GET endpoints. The UI sends these values only on create/update. This prevents accidental exposure through browser dev tools, network logging, or screen sharing — even without an auth proxy. The mask set must mirror the actual destination Secret schema (§6.5): it covers `password`, `ssh-private-key`, `secret-access-key`, `access-key-id`, `account-key` (Azure), and `service-account-json` (GCS). The S3 `access-key-id` was previously absent from the set and leaked in plaintext on GET — masked now because, while an access-key id is semi-identifying rather than secret, returning it to an unauthenticated client narrows credential reconnaissance for no benefit. The Azure `account-key` and the full GCS `service-account-json` were also absent and leaked verbatim — those are outright secrets (full read/write to the bucket holding every dump), so their omission was a real credential-disclosure bug, not a reconnaissance nicety. The recurring lesson: adding a storage backend must extend this mask set in lockstep with the backend's credential keys — deriving the set from each backend's declared secret keys would make the omission impossible.

- **CSRF defence on mutating UI endpoints via `csrfMiddleware`.** The UI has no built-in auth (§3.1) and is commonly fronted by a cookie-based auth proxy; without a CSRF check a malicious page could drive `DELETE /api/destinations/...`, trigger jobs, or rotate age keys through the victim's browser. The middleware (in the global chain, so new endpoints inherit it) lets GET/HEAD/OPTIONS through, then rejects mutating requests whose `Sec-Fetch-Site` is `cross-site`/`same-site`. `Sec-Fetch-Site` is set by all modern browsers and computed client-side, so it survives reverse proxies. For older browsers that omit it but send `Origin`, the Origin host must match `Host` (or `X-Forwarded-Host` for the proxy deployment). Non-browser clients (curl, the restore CLI, monitoring) send neither header and pass through unchanged — this narrows the *browser* attack surface, it does not replace the auth proxy. Token-based CSRF (synchroniser tokens) was rejected: it needs server-side session state the stateless UI doesn't have, and the header-based check is sufficient because the SPA is same-origin. A `Content-Type: application/json` requirement was rejected because bodyless mutations (DELETE, `POST /api/trigger/`) legitimately omit it.
...
- **Bounded `path` label on the request-duration histogram (`normalizeMetricPath`).** `latencyMiddleware` records `backup_operator_http_request_duration_seconds{method,path,status}`. The old normalization collapsed the six parametric CRUD prefixes but recorded every other path verbatim — and because the `/` SPA handler is a catch-all that serves any URL and unknown `/api/*` paths return 404 (still observed), an unauthenticated scanner walking `/wp-admin`, `/.env`, `/a/b/<random>` minted a fresh histogram series per distinct path, each carrying the full bucket set and living for the pod's lifetime: an unbounded-memory / scrape-bloat DoS on the operator's own registry. The UI has no built-in auth (§3.1), so this has to be closed at the metric layer, not assumed away by a proxy. `normalizeMetricPath` now maps to a closed set: parametric routes collapse to their prefix, a hard-coded allow-list of the mux's fixed routes passes through, and **everything else becomes the single label `"other"`**. Adding a new fixed route means adding it to `metricKnownPaths` (or `metricPathPrefixes` if parametric) — otherwise it just shows up under `"other"`, which is a safe default rather than a leak. Regression-tested that a flood of distinct unknown paths collapses to one series.

- **Alerts surfaced via two independent providers.** `internal/alerts.PrometheusProvider` queries `/api/v1/alerts` on the configured Prometheus and filters by `alertname=~"^Backup.*"`; `internal/alerts.LocalProvider` re-evaluates the shipped PrometheusRule conditions inline against the operator's gathered metric registry. The UI handler exposes whichever is configured (Prometheus preferred, local as fallback chained automatically). This keeps the UI useful before kube-prometheus-stack is wired up — most of the value of alerts is "what's on fire right now", which we can answer locally — while preserving the audit-grade path through Prometheus → Alertmanager once it's set up. The two evaluators are deliberately independent: there is no shared PromQL evaluator, the local rules are ~100 lines of Go that mirror the YAML in `values.yaml`. Keep both in sync when adding a rule. The LocalProvider mirrors every *problem-signalling* rule (`BackupOverdue`, `DestinationFailing`, `DumpSizeCollapsed`, `SchemaChanged`, `CharsetChanged`, `StorageCorrupted`, `RetentionFailing`, `AnomaliesAppearing`, `LastRunFailed`, `RestoreVerificationFailed`, `RestoreVerificationStale`); only `BackupSucceeded` is intentionally omitted (it is a positive heartbeat, not a firing condition). The restore-verification rules are keyed on the `mode` label, which the Alert schema has no field for, so it is folded into the summary text — matching how the PrometheusProvider surfaces the rule's `mode` annotation. The `Source` field on each Alert (`"prometheus"` vs `"local"`) tells the UI to show a "for: not honored" disclaimer when in local mode. (Earlier the LocalProvider mirrored only nine rules and silently omitted the two restore-verification rules — a critical `RestoreVerificationFailed` was invisible on the only alert surface a Prometheus-less deployment has; both are now mirrored.)
...
- **Row-counter scan errors fall back to stats, never hard-fail the dump.** `RowCounter.scan()` captured `scanner.Err()` into a field but `Close()` always returned `nil`, so a parse error was silently swallowed. The dangerous consequence was specific to the empty-dump detector: a single dump line exceeding the scanner's 10 MB buffer (`bufio.ErrTooLong` — e.g. a wide row with a large blob under extended-insert) aborts the scan mid-stream, leaving the per-table counts truncated. Those truncated counts then flow into `LooksEmpty`, which would hard-fail an otherwise healthy large-row backup as `dump-empty-content`. Fix: `Close()` now returns `scanErr` (plus an `Err()` accessor for reads after Close — safe without a lock because the `done`-channel close establishes happens-before), and the pipeline checks it: on a non-nil scan error it logs and leaves `dumpCounts` nil so `BuildVerification` falls back to pre/post stats comparison instead of acting on the partial count. Trade-off accepted: in the rare case where the dump is *genuinely* empty AND the scan errored, we miss the count-based check — but the stats comparison still backstops it, and a false-negative on a rare edge beats false-failing every large-row dump. The `Active()`/`Err()` split keeps the three states distinct: inactive (mongo/redis, no parser), active+clean (trust counts), active+errored (distrust counts, fall back).
...
- **Restore-verification Phase 2 spawns the ephemeral DB pod from the worker, owned by the worker pod's UID.** Phase 1's `stream-validate` is a parser test; it cannot tell you "psql would actually accept this" or "mongorestore can rebuild the indexes". Phase 2 modes (`schema-only`, `sample`, `full`) bridge that gap by running an honest restore. The implementation choices that shaped this:
  ...
  - **Pod, not Job.** Jobs add a layer of indirection (the Job controls a Pod) and their TTL semantics are awkward when the parent that owns them (the worker) might exit before the Job's `ttlSecondsAfterFinished` kicks in. We create a bare Pod with `OwnerReference → worker pod`, so K8s GC cascades the delete when the worker pod *object* is removed. **Important correction:** owner-based GC fires on deletion of the owner object, NOT on the worker's process termination — a SIGKILL/OOM'd worker lingers as a `Failed` Pod object (up to the Job's history limit), during which its owned verifier DB pod (holding a full data copy, 2 CPU / 2 Gi, up to 50 Gi emptyDir) would keep running. The happy path still calls `Stop()` explicitly; the backstop for the crash path is `activeDeadlineSeconds` on the verifier pod (`ephemeral.Spec.ActiveDeadline`, default 1 h ≈ the operator's `RUN_TIMEOUT_SECONDS`), so a leaked pod self-terminates instead of running until something else reaps the worker.
  ...
  - **Verifier DB pod runs with a writable root filesystem, unlike the operator/worker.** The PSA ADR below has both operator and worker on `readOnlyRootFilesystem: true`; the verifier DB pod is the one exception. Stock postgres/mysql/mongo images write runtime state to root-fs paths *outside* the data volume at startup — postgres needs `/var/run/postgresql` (socket + lock), mysql `/run/mysqld`, mongo `/tmp` — so under a read-only root fs the entrypoint can't create them and the container crashes straight into `PodFailed` (observed as `ephemeral DB never became ready: ... entered Failed phase`, since the spawner tears the pod down the moment it sees `Failed`). Two fixes were weighed: (a) keep `readOnlyRootFilesystem: true` and mount a writable emptyDir at each engine's runtime path; (b) set `readOnlyRootFilesystem: false` on the verifier container. We picked (b). (a) hard-codes per-image runtime paths into the spawner that drift between image versions — the exact maintenance trap that breaks silently when someone bumps `verification-image`. (b) is one line and works for all four engines. Crucially it does **not** cost PSA-`restricted` compliance: `readOnlyRootFilesystem` is not part of the restricted standard (only `runAsNonRoot` / `seccompProfile=RuntimeDefault` / `drop: ALL` / `allowPrivilegeEscalation: false` are, and those all stay on). The security trade-off is near-zero on this specific pod: it lives seconds, is OwnerRef-GC'd, and only ever holds a copy of data the cluster already has. The only environment where (b) is wrong is a cluster with an admission policy (Kyverno/Gatekeeper) that *enforces* `readOnlyRootFilesystem: true` — there the verifier pod is rejected at admission and you must supply the writable mounts of approach (a) instead.
...
- **SSRF egress guard on UI-initiated storage dials (`checkDestinationEgress`).** The UI has no built-in auth (§3.1) and can create destinations, so a caller could point a destination's `host` (sftp/ftps) or `endpoint` (s3) at `169.254.169.254` (cloud metadata / IMDS) or an internal service and use the differentiated `test-connection` / `destination-stats` / `destination-health` / `consistency-check` errors as a reachability oracle from the operator's network position. Fix: a guard at `storageForPool` — the single choke point every UI storage path routes through (diagnostics, `/download`, dashboard probes, `listTargets`) — resolves the user-controllable host and refuses to build the client (so the dial never happens) when it points at a blocked range: loopback, link-local (covers 169.254.0.0/16 and fe80::/10 IMDS), the AWS IPv6 metadata address (`fd00:ec2::254`, a ULA the link-local checks miss), unspecified, and multicast. RFC1918 / generic ULA are deliberately NOT blocked — in-cluster MinIO, a LAN NAS, and internal SFTP hosts are the operator's core use case; constrain those further with the opt-in NetworkPolicy. Placed at the construction choke point rather than per-handler because construction is where the (unauthenticated) request first turns into an outbound client, and putting it there covers every current and future UI dial path in one spot. Trade-offs: (a) azure/gcs hosts are derived subdomains of fixed cloud domains, so they have no attacker-chosen host to check; (b) DNS rebinding (TOCTOU) can still re-resolve a name to a blocked IP between the check and the storage layer's own dial — the guard raises the bar against the trivial literal-IP / metadata-hostname attack but is defence-in-depth, not a NetworkPolicy substitute; (c) on DNS resolution failure the guard does not reject, letting the real dial surface its normal sanitized error. The worker's fan-out dials are intentionally out of scope (they run from Secrets and upload only age-encrypted ciphertext; their egress is the NetworkPolicy's job). Regression-tested for metadata/loopback rejection and RFC1918/AWS allow.

- **FTPS `mkdirAll` restores the working directory after its existence probe.** `mkdirAll` recurses each path segment; when `MakeDir` returns 550 (ambiguously "already exists" or "permission denied") it probes with `ChangeDir` to tell them apart. But `ChangeDir` **mutates the session CWD** and the old code never restored it, while every `MakeDir`/`Stor` here uses a full path resolved against the login directory. So the first "segment already exists" probe left the CWD inside the tree, and the next `MakeDir` targeted `<cwd>/<full-path>` — a doubled prefix (`~/t` → `MakeDir("t/2026")` → `~/t/t/2026`) whose parent doesn't exist → 550 → the whole create failed. This broke the Phase-2 meta upload (a fresh connection that re-runs `mkdirAll` on an existing tree) on **every** FTPS destination whenever the `path-prefix` was relative or empty; an absolute prefix masked it because absolute paths don't depend on CWD, and it also poisoned the shared `WithSession` connection for later List/Delete. Fix: capture `CurrentDir()` before the probe and `ChangeDir` back immediately after, so every segment's `MakeDir` runs from the login dir. If the server doesn't support `PWD` we can't restore, so we skip the probe and treat 550 as "probably exists" (letting `Stor` surface any real problem) rather than corrupt the session. `mkdirAll` now takes a 3-method `ftpDirClient` interface so the CWD logic is unit-tested with a fake server (existing-dir → CWD-restored + no doubled path; no-PWD → probe skipped; genuine failure → surfaced) without a live FTP server.
...
- **UI cache background-refresh goroutine recovers panics (`safe.Goroutine`).** The stale-while-revalidate cache (`ui/cache.go`) spawns a goroutine in `getOrRefreshAsync` that runs a caller-supplied `load()` closure doing storage I/O + `meta.json` parsing. That goroutine was the one long-lived operator goroutine NOT wrapped in `safe.Goroutine` — every other one already was (the §-above `safe.Goroutine` ADR established the contract: "long-lived operator goroutines must recover"). `net/http` recovers panics *per request*, but NOT in a goroutine a handler spawns: the SSE-driven dashboard polls `/api/targets` and `/api/dashboard/heatmap` ~every 10 s, each fans out background refreshes, and a panic in `load()` (nil deref on a malformed meta, an unexpected storage response) crashed the **whole operator pod** — observed in the field as "the operator restarts sometimes". Fix: `defer safe.Goroutine(c.log, "ui-cache-refresh", c.name+":"+key)` at the top of the goroutine. Second, subtler fix bundled in: the `delete(c.loading, key)` that clears the per-key dedup flag ran *after* `load()`, so even a recovered panic would have pinned the key as "loading" forever (it would never refresh again — silent stale-forever). Moved into a `defer` so it always runs. Defer order is load-bearing (LIFO): `safe.Goroutine` is registered first so it runs last, recovering after the loading-flag and semaphore-slot defers have released. The cache carries a `log logr.Logger` + `name string` purely for this recovery log; a zero `logr.Logger` is a no-op sink, so tests that build caches without one still recover (just don't log).

- **Target detail page uses stale-while-revalidate for run history, mirroring the dashboard.** Run history is the *only* storage-probing fetch on the detail page (the backend `target()` probes each destination, up to 12 s apiece). The page used to `await` it inline before writing ANY content — so clicking a source showed a blank spinner for many seconds even though the summary cards (config, latest run) come from the fast, non-blocking `/api/targets` read, and a language switch (`setLang` → re-render) re-fired the whole slow probe. Two symptoms, one cause: the slow probe sat on the synchronous render path. The dashboard already solved this for its charts via `_fleetSummary`/`_slowProbes` caches (render from cache, refresh in background, repaint via SSE); the detail page now does the same. New `_targetRuns` cache (30 s TTL, keyed by target name) + per-name in-flight guard. `renderTargetDetail` renders the summary immediately and drops a `<div id="target-runs-region">`; `fetchTargetRuns` refreshes in the background and repaints ONLY that region, never the whole page (so an in-flight probe can't yank the summary away). A purely presentational re-render — language switch, sort, an unrelated SSE tick — reuses the cache and never re-probes storage; that is what made re-translating the page slow. A probe is kicked only when the cache is for a different target, expired, or invalidated (SSE `meta_changed`/`backup_triggered` set `_targetRuns.lastFetch = 0`). **Loading vs. failed vs. genuinely-empty:** a flat runs array can't distinguish "probe timed out → empty" from "this target has never run". The fix: `targetRunsRegionHTML` shows a spinner while in-flight, and when the result is empty it checks whether the target's `Latest` meta exists — if it does, at least one run MUST exist, so an empty list there is a probe failure (shows "run history unavailable" + a Retry button), not the misleading "No runs" it used to show. Only a target with no `Latest` and a clean fetch renders "No runs". Trade-off: the detail page can briefly show stale run history after a new backup lands; the SSE-driven invalidation + repaint corrects it within seconds — same stale-window trade the dashboard already accepts. The backend `target()` stays blocking-but-bounded (its own ADR above) — this change takes it off the *frontend's* critical render path, it doesn't change the endpoint.

- **Meta-upload-on-all-destinations failure fails the run, not silently succeeds.** `uploadMeta` is best-effort (retries 3× per destination) but used to be fire-and-forget: if the dump uploaded but every destination then failed the small `meta.json` upload, the pipeline still called `SetLastRunStatus(true)` and returned nil. Because the metrics refresher, UI, and restore listing ALL key off `meta.json`, that run was invisible — a phantom backup (dump objects present, no sidecar, un-analyzable, only surfaced 36 h later as `BackupOverdue`). Fix: `uploadMeta` now returns `(attempted, succeeded)`. `succeeded==0 && attempted>0` is treated as a run failure (`SetLastRunStatus(false)`, `BackupFailed` Event, worker exits 1) so K8s retries with a fresh timestamp — a visible failure beats a silent phantom. Considered alternatives: (a) re-upload meta on the next run only — rejected, leaves a window where the dump is unrecoverable-by-tooling; (b) keep best-effort but emit a metric — rejected because worker metrics aren't scraped (the whole reason the refresher reconstructs from storage). A *partial* failure (sidecar reached ≥1 destination) keeps the run successful — it's still discoverable — and emits `BackupMetaPartial` (warning) so per-destination divergence is auditable. Meta-upload failures now log at Error, not Info. Trade-off accepted: the orphan dumps from a fully-failed attempt linger until retention prunes them by timestamp.

- **schema-only restore-verifier propagates scanner errors instead of masking them as EOF.** `filterPostgresSchemaOnly` / `filterMySQLSchemaOnly` pipe the decrypted dump through a `bufio.Scanner`, dropping data lines so the verifier replays DDL-only. The goroutine used `defer pw.Close()` — a clean EOF — and never checked `s.Err()`. So a scan failure (`bufio.ErrTooLong` on a line > 32 MB, e.g. a wide row or a *truncated/corrupt decrypted stream that never yields a newline*) silently stopped the scan; psql/mysql read the partial DDL, exited 0, smoke queries passed, and the verifier reported `match` on a truncated dump — defeating the entire point of restore verification. Fix: check `s.Err()` after the loop and `pw.CloseWithError(s.Err())` (which is `Close()`/EOF when err is nil), and `CloseWithError(err)` on write failures too, so the consuming client aborts. Same bug class the RowCounter ADR already fixed for the empty-dump path. Regression-tested with a reader that fails mid-stream.

- **Degraded empty-dump detection is surfaced as a warning, NOT enforced via an absolute byte floor.** When `CollectStats` is unavailable (analyzer disabled, or stats query failed — e.g. redis AUTH on `INFO`), `BuildVerification` returns `skipped` and the empty-dump heuristic can't fire: SQL compares stream row counts against `preStats`, mongo/redis compare encrypted size against `preStats` total, and all need the baseline. A header-only redis/mongo dump (server exited 0 but emitted nothing) would then pass since only `encryptedSize == 0` hard-fails. The obvious fix — an absolute byte floor independent of `preStats` — was rejected: `encryptedSize` is the size *after* age encryption, whose header scales with recipient count (~60 + N×130 bytes), so any fixed floor either misses real failures or false-fails legitimate tiny dumps. Per the project's own "false-positives are worse than rare misses" principle (the heuristic-threshold ADR), a noisy hard-fail is the wrong trade. Instead, when `EmptyDumpCheck && preStats == nil` the run emits `BackupEmptyCheckDegraded` (warning) + logs, so the gap is visible and an operator can act, without risking a false hard-fail on a healthy backup. Engine-agnostic (keyed only on `preStats == nil`), so it also respects the §8.2 "don't branch on type strings outside the factory" rule.

- **`meta.List` / `LatestPerTarget` share one connection across the List + per-file Gets.** Both enumerate a prefix then `Get` each `*.meta.json` to parse it. On SFTP/FTPS every `Get` dials a fresh SSH/TLS connection (~100-300 ms handshake), so a target with N runs cost 1 + N handshakes — and the UI hot paths (`target()`, `fleetHeatmap`, `estimateDuration`, `listTargets`) hit this constantly. The metrics refresher's `loadLatestMeta` already fixed the same pattern via `WithSession`; these two were missed. Fix: type-assert `storage.BatchStorage`, open one session, route the List + all Gets through it (S3 has no session concept and falls through to per-call dialing unchanged). Unlike the refresher's version, a session-open failure falls back to per-call dialing rather than returning nil — the UI prefers stale-but-present data over a blank page.

- **Storage scrubber walks sources concurrently behind a shared egress throttle.** The scrubber re-streams each dump through SHA256 to detect bit-rot. The outer source loop was strictly serial — only destinations *within* a source ran in parallel — so one source with a slow/large destination blocked every source behind it, and a tick on a big fleet could exceed the 24 h interval (the exact per-source-serial blowup the metrics-refresher iteration-3 ADR already fixed for itself). Fix: a global source-worker pool (8) walks sources concurrently, while a single shared stream semaphore (`defaultScrubConcurrency`=2) bounds total in-flight full-dump re-streams across the whole tick — egress stays capped at the previous level, but the per-source barrier is gone, so a freed slot is immediately reused by whichever source is ready rather than waiting for the current source to finish. Did not raise the stream cap: egress is the real cost and the win here is scheduling fairness, not more bandwidth.

- **Operator/UI hardening: bounded slot map, race-free reconciler client, single SSE reconnect.** Four latent issues with no observed failure yet, fixed pre-emptively. (1) `MetricsRefresher.perDestSlots` (the per-destination concurrency-semaphore map) grew unbounded under destination churn — every distinct name ever seen leaked one channel + entry for the operator's lifetime. Now pruned each tick via `retainDestSlots`, mirroring `Pool.Retain`; safe to delete mid-tick because the refresh only fans out to current destinations, so an absent name has no in-flight goroutine and deleting the map entry never closes a live channel. (2) `RecipientReconciler.rebuildMergedWith` swapped the shared `r.Client` field (`r.Client = c; defer restore`) — safe only because `Bootstrap` runs before the manager starts, but a latent data race if it ever overlapped a reconcile. `rebuildMerged` now takes the client as a parameter; the field swap and the wrapper are gone. (3) `connectSSE` could stack reconnect timers / leak `EventSource`s on a flapping server (its own auto-reconnect races the manual one) — now guarded by a single `_sseReconnectTimer` and detaches the old source's `onerror` before closing. (4) `badge.running` was missing from the i18n dictionaries (rendered via a German-literal `||` fallback); added to en/de/fr and the suspended case reuses the existing `badge.paused` key.

- **Target detail run history is paginated client-side and the size-trend chart is capped.** A high-frequency schedule (e.g. `*/15`) with long retention accumulates tens of thousands of runs; the detail page rendered one table row (~11 cells) AND one size-chart `<circle>` per run, which froze the browser. Both are now bounded WITHOUT changing the fetch: the run-history table pages client-side over the already-cached `_targetRuns` array (`RUN_HISTORY_PAGE_SIZE`=50, prev/next + "{from}–{to} of {total}"), and the size-trend chart receives only the newest `MAX_TREND_POINTS` (365) points with a "most recent N" sub-label when truncated. The status heatmap is left full — it aggregates by day (≤91 cells) so run count drives a cheap JS loop, not DOM. The fetch is deliberately NOT bounded: `target()`, `fleetHeatmap`, and `estimateDuration` share the `name@dest` `runsCache` and the heatmap needs its full day-window, so a count-bound fetch would either truncate the heatmap or require a second cache population that regresses the common retention==window case (see the dropped "meta.List limit/since" investigation). Pagination is pure rendering: `gotoRunHistoryPage` repaints only `#target-runs-region` from cache, never refetches; `_runHistoryPage` resets when the viewed target changes. Trade-off: older runs aren't all visible at once, but the total count is shown and every run remains reachable by paging (and via the restore CLI). The full run array stays in memory — this caps render cost, not data availability.

- **String-prefix storage backends (S3/Azure/GCS) join paths by concatenation, not `path.Join`.** `full()` used to be `strings.TrimLeft(path.Join(pathPrefix, p), "/")`. `path.Join` runs `path.Clean`, which strips the trailing slash a List prefix carries for target isolation: `full("db/")` collapsed to `prefix/db`, and because S3/Azure/GCS List is a pure string-prefix match, that also matched the sibling target `prefix/db-archive/...`. The consequences were severe and silent: retention for `db` (which round-trips the bled paths correctly through `stripPrefix`) deleted `db-archive`'s dumps — even with `db-archive` set to `retention-days=0` (keep forever) — and the analyzer baseline / `last_success` metrics for one target were populated from another's meta. The restore CLI's `--purge` inherited the same bleed, so a DSGVO-erasure for one customer could destroy another's artifacts. Fix: `full()` concatenates `pathPrefix + "/" + p`, preserving the caller's trailing slash exactly (Upload/Get/Delete pass paths without a trailing slash, so they are unaffected), and `stripPrefix` trims the full `pathPrefix + "/"` boundary instead of the bare prefix (so a sibling `prod-eu/...` under prefix `prod` is never mangled into `-eu/...`). SFTP/FTPS were never affected — they walk a directory tree, not a string prefix. Object stores treat a literal `..` as an inert key component, so dropping `path.Clean` here does not introduce traversal; traversal via the target *name* is closed separately at the parser (below).

- **Structural identifiers (source target name, destination name, path-prefix) are validated at parse time.** `TargetName` and destination `Name` come from the `backup.mogenius.io/name` annotation and flow into `path.Join`-built object keys, metric labels, and CronJob names; `path-prefix` is the object-key root. None were validated, so a source annotated `name: ../../other-tenant` wrote (and let retention delete) outside the destination's prefix — defeating the §6.4 isolation contract. `ParseSource`/`ParseDestination` now reject names not matching `^[A-Za-z0-9][A-Za-z0-9._-]*$` (no `/`, no leading `.`, ≤253 chars) and any `path-prefix` containing a `..` segment. This is a hard rejection (the Secret is skipped), not the forgiving-fallback treatment feature-flag annotations get — a name is structural, like the required `host`/`username` keys, so a bad one must fail loudly rather than be silently rewritten. Behaviour change: existing sources with a `/` or space in the name annotation (already broken in the hash router and questionable in paths) now stop producing CronJobs until renamed.

- **Retention MinKeep counts real dumps, not timestamps.** Failed runs write a `dump-<ts>.meta.json` sidecar with no dump file; `groupByTimestamp` bucketed those as full timestamps, so `selectForDeletion`'s "keep the MinKeep newest" floor could be entirely consumed by a streak of failure sidecars at the top of the list — pushing every real dump below the floor and letting the age sweep delete all recoverable backups, precisely during the outage when they are needed. Fix: the floor now only decrements for groups that contain an actual dump artifact (`groupHasDump`); failure-only groups are never floor-protected and are still age-pruned like orphan metas. Regression-tested (`TestSelectForDeletion_FailureMetasDoNotConsumeFloor`).

- **`loadLatestMeta` returns a typed outcome so `destination_failed` can catch runtime storage failures.** The function used to collapse "no meta uploaded yet" and "storage unreachable / unreadable" into a single `found=false`. The refresher's per-destination goroutine `return`ed on `!found` without touching the gauge, and because `Pool.Get` only *constructs* a client (SFTP/S3 constructors never dial — the connect/auth happens later in `List`/`Get`/`WithSession`), a rotated password or an offline host left `destination_failed` at its last value (0 = healthy). `BackupDestinationFailing` — documented in §14 as the "one destination down" alert — could therefore never fire for that exact failure mode; the only path that ever set the gauge to 1 was a config-parse error in the factory. Fix: `loadLatestMeta` now returns `metaOK` / `metaEmpty` / `metaError`. The refresher sets `destination_failed=1` on `metaError` (I/O failure or unparseable payload), `0` on `metaEmpty` (reachable, nothing uploaded — the legitimate first-run case) and `metaOK`. The scrubber treats anything other than `metaOK` as "nothing to scrub". Trade-off: a corrupt-but-present meta now counts as a destination failure rather than being silently ignored — intentional, since a destination serving unparseable data is not healthy.

- **UI stored-XSS in the run-history download/view controls, fixed by correct-context escaping.** Two spots built `onclick="viewMeta(...)"` / `onclick="copyToClipboard(...)"` with the wrong escaper for a JS-string-inside-HTML-attribute context: `renderDownloadLinks` used `escHTML` (which escapes `<>&` but NOT `'`/`"`, per its own contract) on the free-form destination `name` and the run timestamp, and the meta-viewer inlined raw `JSON.stringify(data)` (full of unescaped `"` and `<`) into the Copy button's `onclick`. Since target and destination names come from `backup.mogenius.io/name` annotations and meta content includes DB-controlled table names, an attacker who controls only DB content or a Secret annotation could land markup that executes with the operator's UI privileges (Secret CRUD, trigger, exfiltration via a rogue destination). Fix: `renderDownloadLinks` uses `escJS` for every onclick argument and `encodeURIComponent` for every URL path segment; the meta-viewer Copy button is bound via a JS closure (`data-action="copy-meta"` + `.onclick = ...`) instead of interpolating JSON into an attribute. The broader pattern (delegated listeners + `data-*` over string-built `onclick`) is the durable fix and is noted as a follow-up; the remaining `onclick` sites use constrained values (K8s Secret names, fixed enums) and are not injectable.

- **UI S3 destination form used the wrong data-key names, and masked credentials must be preserved-on-empty across all backends.** The S3 form wrote `access-key` / `secret-key`, but the backend requires `access-key-id` / `secret-access-key` (`storage/s3/s3.go`) — so *every* S3 destination created through the UI produced a Secret the backend rejected at first use. It also hard-required `endpoint` (wrong for AWS, which infers the endpoint from the region) and offered no `path-style` toggle (needed for MinIO). Fixed the key names, made `endpoint` optional with a hint, and added a `path-style` checkbox. Coupled fix: once §18's masking change made Azure `account-key` and GCS `service-account-json` return as `***` on GET, the edit form (which leaves masked fields blank and relies on PUT-merge to preserve them) would have *deleted* those credentials on save unless the key is in the frontend's `DEST_SENSITIVE_KEYS` set — so `access-key-id`, `account-key`, and `service-account-json` were added there, and the sensitive credential inputs dropped their `required` attribute (which would otherwise block edit-mode submit when the field is intentionally blank), matching the long-standing SFTP-credential convention of "label carries `*`, no `required` attribute".

- **Phase-2 verifier DB pod uses a per-run random password, not a compile-time constant.** All four restore engines hard-coded `verifierPassword = "verifier-ephemeral"` (redis had its own copy) into the ephemeral pod's root credential, the probe, the restore CLI env, and the smoke-query DSN. The pod holds a full copy of the source's data and — with the ephemeral-pod-spawn RBAC enabled — is reachable on the flat pod network for the minutes-to-hours of a restore, so any workload in any namespace could connect with a credential that was effectively public and read everything, bypassing the age-encryption model. Fix: `NewEngine` generates a fresh 16-byte hex password per verification run (per `factory.New` → `restore.New` → `NewEngine`, which is per-run) and injects it into each engine struct; the probe is now an engine-bound closure and `PodSpec`/`Restore`/`SmokeQueries`/`sanitiseStderr` all read the instance field. A leaked or observed password now grants access to a single short-lived pod holding one run's data, never the fleet. This does not remove the value of a NetworkPolicy on the verifier pod (defence-in-depth), but it closes the "known credential" hole on its own. Regression-tested (`TestNewEngine_UniquePasswords`, `TestRandomPassword_UniqueAndHex`).

- **DB-connection TLS knobs apply to the dump tool, not just the stats connection.** `extra-sslmode` (postgres) / `extra-tls` (mysql) were honoured only by the in-process driver used for `CollectStats`; the actual bulk-data path (`pg_dump`, `mysqldump`/`mariadb-dump`) was launched with only the password, so `pg_dump` ran on libpq's default `prefer` and could silently downgrade to plaintext against a MITM'd or misconfigured server even when the source asked for `verify-full`. Fix: postgres exposes a shared `sslMode()` used by both `connString()` and the pg_dump env (`PGSSLMODE`, plus optional `PGSSLROOTCERT`/`PGSSLCERT`/`PGSSLKEY` from `extra-ssl*`); mysql maps `extra-tls` to the client-appropriate flag via a pure, unit-tested `tlsDumpArgs` — Oracle's `mysqldump` gets `--ssl-mode=…`, MariaDB's `mariadb-dump` gets `--ssl`/`--skip-ssl` (the same binary-divergence handling as `--column-statistics`, keyed on a cached `--version` banner probe). Empty/`preferred` adds nothing, so existing deployments are unchanged. Certificate *identity* verification (`VERIFY_CA`/`VERIFY_IDENTITY`, MariaDB `--ssl-verify-server-cert`) is only emitted when a CA path is provided (`extra-ssl-ca`, or postgres `extra-sslrootcert`) — that needs a file mounted into the worker pod; without one, an enabled TLS value still enforces an encrypted channel (`--ssl-mode=REQUIRED` / `--ssl`), which is what closes the plaintext-downgrade hole. Regression-tested (`TestTLSDumpArgs`, `TestBuildDumpArgs_EnforcesTLS`, `TestSSLMode_SharedByStatsAndDump`).
