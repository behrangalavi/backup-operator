...
|---|---|---|
| Worker → Database | DB-native (pg, mysql, mongo) | TLS if configured via `extra-sslmode` (postgres) / `extra-tls` (mysql/mariadb). Applies to BOTH the stats connection AND the dump tool (`pg_dump` via `PGSSLMODE`, `mysqldump`/`mariadb-dump` via `--ssl-mode`/`--ssl`). Certificate verification (`verify-ca`/`verify-full`) additionally needs `extra-ssl(root)cert`/`extra-ssl-ca` pointing at a CA file mounted into the worker pod; without one, an enabled TLS value still enforces an encrypted channel. |
