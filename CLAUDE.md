...
- **Sensitive field masking in API GET responses.** Passwords, SSH private keys, and S3 secret keys are returned as `***` in GET endpoints. The UI sends these values only on create/update. This prevents accidental exposure through browser dev tools, network logging, or screen sharing — even without an auth proxy. The mask set must mirror the actual destination Secret schema (§6.5): it covers `password`, `ssh-private-key`, `secret-access-key`, and `access-key-id`. The S3 `access-key-id` was previously absent from the set and leaked in plaintext on GET — masked now because, while an access-key id is semi-identifying rather than secret, returning it to an unauthenticated client narrows credential reconnaissance for no benefit.
